...
Debugged and fixed three issues found while actually using the deployed app (real MongoDB Atlas + Redis Cloud), not part of a numbered phase:

1. **Run Code was slow.** `SyncJudgeService` opened a brand-new `httpx.Client` (fresh TCP+TLS handshake) on every single Judge0 call, including every parallel test case inside a Submit - now a single pooled, keep-alive client is reused per worker process. Separately, `run_job_timeout_seconds` (45s) was shorter than Judge0's own worst-case retry budget (~90s), so a slow-but-recoverable response got killed by RQ mid-retry instead of failing cleanly - both job timeouts were realigned with the (also tightened, 2→1) retry budget, and enqueue/queue-wait/Judge0-call/total timing is now logged at every stage.
2. **Submit Code looked broken after an auto-submit/lock.** The frontend's local round-session state never refreshed after a 409 (session no longer active), so a correctly-rejected submission just showed a generic error with the button still enabled. `ProblemDetail.tsx` now refreshes the session on any round-scoped 409/404, immediately reflecting the true locked state.
3. **The tab-switch warning only appeared after a long absence.** Violation counting was gated behind the grace period at return time; it's now immediate and unconditional on `visibility_hidden` (any duration, even under a second), while the grace period was re-scoped to a genuinely separate mechanism - a client-side timer that, if the tab is still hidden when it elapses, triggers a server-reverified prolonged-absence auto-submit, completely decoupled from violation-count-based locking. `window_blur`/`window_focus` were dropped as violation triggers (kept only for the audit log) since they fired alongside `visibilitychange` for the same switch and could double-count it.

Verified with 9 new checks covering the new violation/grace-period semantics, a full re-run of the Phases 10-13 regression suites (all passing), and a live load-test/real-Judge0 pass with zero errors. See the code comments in `coding_round_service.py`, `judge_service.py`, and `ProblemDetail.tsx`'s visibility-monitoring effect for the detailed reasoning behind each fix.
